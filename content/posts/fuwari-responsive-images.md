...
有关具体的属性设置不再赘述，唯一需要注意的是`isRemote`行的`inferSize={true}`，对于远程图像，这是必须的属性，因为我们不能在这个需要复用的组件中指定宽度和高度。
...
改造后，`ImageWrapper.astro`就可以接受我们在其他组件中传入的`widths`、`quality`和`sizes`属性了。
